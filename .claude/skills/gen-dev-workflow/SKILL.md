...
```text
    ...
    → 處理完畢 → 呼叫 reviewer agent 重新審查 [Model: Fable]
...
### Fable 優先原則（最高推論等級的選模規則）

所有需要**最高推論等級**的環節（planner、reviewer、STAGE 2 的「最強 model」分級）一律**優先使用 Fable**（`model: "fable"`），僅在 Fable 不可用時才 fallback 回 Opus：

```
派發需要最強推論的 Task / agent 呼叫
  → 先帶 model: "fable"
  → 若呼叫因 model 不可用而失敗（model 不在可選清單 / 回傳 model 相關錯誤）
     → 改帶 model: "opus" 重派同一 prompt（僅此一次 fallback，不再降級）
  → 其他原因的失敗（任務本身錯誤）不觸發換 model，走原本的 retry 路徑
```

下表與流程圖中標註 `Fable` 的位置皆隱含此 fallback；標註 `Sonnet` 或「快/便宜」的位置**不受影響**，維持原樣。

### Stage 層級的基準分配

| Stage | Agent | 基準 Model | agy 委派 | 不委派的原因 |
|-------|-------|-----------|------------|------------|
| 0a/0b 規劃 | planner | Fable（fallback: Opus） | — | 設計與計畫拆解是最高槓桿推論，錯了後面全錯 |
| 1 建立分支 | brancher | Sonnet | ✦ gh issue create, git checkout | 純 IO |
| 2 實作 | implementer | **見下方分級** | ✦ 代碼+測試+commit（Claude 驗收）| — |
| 3 審查 | reviewer | Fable（fallback: Opus） | — | 根因判斷需最強推論，且不該讓產出代碼的同源 model 自審 |
...
| 需設計判斷或廣泛 codebase 理解 | 最強 model（Fable，fallback: Opus） | 重構狀態機、新增跨層架構 |
...
> 不變式：審查報告由 reviewer（Fable，不可用時 fallback Opus）親自產出，**不委派 agy**。多 angle 只是提高召回率的輸入，不取代 reviewer 的最終判斷。退回 STAGE 2 的條件與層級不變。
