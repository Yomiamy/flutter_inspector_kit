...
| **STAGE 1 斷裂** | `promote` 後 `stage-done 1` 恆遭拒（Bug 1.6） | 🔴 高 | `promote` 只填 `branch` 欄位不推進 `stage`（維持 `0a`），sequence 模式下 `stage-done 1` 被 guard 擋下（`0a != 1`）。正常 sequence 流程的 STAGE 1 happy path 斷裂，目前僅能用 `--mode jump` 繞過（喪失轉移表保護）。詳見 [`docs/brainstorm/2026-07-30-workflow-brainstorm.md`](../brainstorm/2026-07-30-workflow-brainstorm.md) §6。 |
...
**最該修的問題（更新後）：** **Bug 1.6 STAGE 1 斷裂**。`promote` 後 `stage-done 1` 在 sequence 模式下必然被擋（根因：`promote` 不推進 `stage`，維持 `0a`），迫使流程退縮到 `--mode jump` 繞過，喪失轉移表保護。首選修復方案為修改 SKILL.md 的 STAGE 1 收尾流程（promote 後依序 `advance 0b --confirmed` → `advance 1 --confirmed`），零腳本改動。其次是暫停點密度：正常路徑 6 個固定暫停點、STAGE 2 逐任務再線性膨脹——「自動驅動」的承諾被密集確認打斷，應考慮讓使用者選擇確認粒度。
