...
> `docs/architecture/2026-08-08-wf-state-harness-guardrail.md`「同構的第二個弱點：委派的工作目錄約束」
...
- **U-2｜稽核紀錄的位置與生命週期**：寫在 `.claude/workflow-state/` 旁、或獨立 log 檔？多久清一次？（AC-B5 只要求「可稽核」，不指定形式。）
...
- **U-5｜白名單的表達位置**：寫死在 hook 腳本內、或抽成外部設定檔？抽出來可維護但多一層；寫死簡單但每次調整要改腳本。（Ponytail 傾向：先寫死，出現第二個消費者再抽。）
