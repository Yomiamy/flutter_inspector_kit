...
## 5. 已知缺口（變更前盤點 — Historical Snapshot）

調查交叉比對發現目前 working tree 的兩檔有以下落差，需在實作計畫中決定處理方式：

1. **G-1（內容缺口）**：`styleguide.md` 的「文學體裁**範例詩句**」（如「變數命名亂如麻…」五言絕句等 4 段範例）**未搬移到任何檔案**。目前 `.pr_agent.toml` 的 `extra_instructions` 只有規範定義、無範例。→ 影響 AC-5，會降低 AI 文學收尾品質。**✅ 已由本 PR 實作解決**（範例詩句已加入 `.pr_agent.toml` 的 `extra_instructions`）。
2. **G-2（涵蓋度）**：`best_practices.md`（136 行）是 `styleguide.md`（550 行）的精簡版，部分子小節的具體範例程式碼被省略。需確認精簡是否丟失了審查所需的關鍵判準（如 Singleton 範例、建構式預設值兩種寫法）。**✅ 已排除**（經評估，精簡版已涵蓋審查所需核心判準）。
3. **G-3（路線標註）**：v1/v2 分歧的風險目前只在對話中說過，**未寫進 repo 任何檔案**。需在設定檔或文件補一句書面說明，避免日後誤解。**✅ 已由本 PR 實作解決**（v1/v2 註解已加入 `.pr_agent.toml` 與 `best_practices.md`）。
4. **G-4（文件提及，可選）**：`README.md` 未描述 code review / PR 流程。可選擇性補一行指向設定檔。**🔲 未實作**（低優先度，暫不處理）。
