...
- **狀態**：STAGE 0b — 待確認
- **已確認的開放問題**：節流固定 2 秒（不開 API）｜接受 channel 換 ID／刪除重建的副作用｜驗收以 AOSP/Pixel 為基準
...
├── final Duration window          // 固定 const Duration(seconds: 2)
...
- **時鐘注入**（`DateTime Function()`）是可測試性的關鍵：單元測試用假時鐘推進時間，驗證「1 秒內 20 筆只有 1 次 alert」「跨 2 秒窗口後再次 alert」，零 sleep、零 flaky。
...
- **TDD 順序**：先寫測試（紅）→ 實作（綠）：
  ...
  3. 假時鐘推進 1.999 秒 → `false`；推進至 ≥2 秒 → `true`。
  4. 「1 秒內 20 筆」連續呼叫只有第 1 筆 `true`（US-2 驗收第 3 條）。
  5. `true` 之後窗口重新起算（連續兩次跨窗 alert 間隔 ≥ window）。
- **實作**：第 1.1 節的結構，`window` 預設 `const Duration(seconds: 2)`，建構子允許覆寫 window 與 now（僅供測試，不對外 export）。
...
- **README**：Live notification 段落補述——通知以 heads-up 橫幅浮出（Android）/ 前景 banner（iOS）、2 秒節流、無聲、升級自動遷移 channel（使用者對舊 channel 的手動設定不保留）。
...
- **實機驗證清單**（example app，AOSP/Pixel 基準，對應規格第 5 節）：
  ...
  - [ ] 連續高頻請求（example 的 burst 按鈕或連點）：2 秒窗口內僅靜默更新，內容/計數正確；跨窗口再次浮出。
  - [ ] 通知權限拒絕：no-op 不崩潰。
