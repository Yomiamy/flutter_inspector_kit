...
## 目標
依照 architecture report 的階段三指示，整理 UI 中散落的硬編碼 const 邊距、字體、顏色等樣式，統一移至 `InspectorTheme`。同時確認代碼中不存在危險的 `.firstWhere` 呼叫。此外，進行架構整潔度（Taste）的調整，將 `InspectorOverlayManager` 移動至 `core/` 目錄。
...
### Task 6: 調整 OverlayManager 的放置位置 (結構優化)
* **內容**：將 `InspectorOverlayManager` 與其測試檔案移至 `core/` 以維持架構整潔。
* **修改**：
  * 將 `lib/src/ui/inspector_overlay_manager.dart` 移動到 `lib/src/core/`
  * 將 `test/ui/inspector_overlay_manager_test.dart` 移動到 `test/core/`
  * 修正對應的 import 與測試。
* **狀態**：✅ 已完成。
